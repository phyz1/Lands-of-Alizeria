/datum/virtue/utility/riding
	...
	desc = "Я умею ездить верхом на всех видах животных, и у меня с одним из них особенно крепкая связь, позволяющая мне вызывать его издалека и при необходимости отправлять восвояси. Если мой драгоценный спутник когда-нибудь умрет - мне будет грустно."
	custom_text = "Предоставляет возможность выбрать тип маунта, которого нужно призвать на свою сторону, и, кроме того, дать ему имя. Благородные персонажи могут выбирать лошадей. Получает две способности, позволяющие отправлять коня прочь и вызывать его обратно по мере необходимости (только на открытом воздухе). Если выбранный маунт погибает, -10 к настрою до конца раунда (не может быть восстановлен ни при каких обстоятельствах)."
	added_skills = list(
