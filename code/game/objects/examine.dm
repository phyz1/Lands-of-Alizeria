...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	if(real_value > 0)
		if(HAS_TRAIT(user, TRAIT_SEEPRICES) || simpleton_price)
			. += span_info("Ценность: [real_value] медяков")
			...
			. += span_info("Ценность: [fumbled_value] медяков... <i>I think</i>")
...
/obj/item/proc/integrity_check(elaborate = FALSE)
	...
	if(obj_broken)
		return span_warning("Это сломано")
	switch(int_percent)
		if(1 to 15)
			result = span_warning("Это скоро сломается")
		if(16 to 30)
			result = span_warning("Это сильно повреждено")
		if(31 to 80)
			result = span_warning("Это повреждено")
		if(80 to 99)
			result = span_warning("Это немного повреждено")
	return result
