...
#define ALIZERIA list(\
	"Spirit Map (VM)" = 'sound/alizeria/Spirit_Map_VM.ogg',\
	"Shillings (FH)" = 'sound/alizeria/Shillings_FH.ogg',\
	"Secret Song (FH)" = 'sound/alizeria/Secret_Song_FH.ogg',\
	"Memories (MF)" = 'sound/alizeria/Memories_MF.ogg',\
	"Memories (FH)" = 'sound/alizeria/Memories_FH.ogg',\
	"Maria Theme (MF)" = 'sound/alizeria/Maria_Theme_MF.ogg',\
	"Live Forever (VM)" = 'sound/alizeria/Live_Forever_VM.ogg',\
)
...
/obj/structure/roguemachine/musicbox/attack_hand(mob/living/user)
	...
	if(button_selection=="Change Song")
		var/songlists_selection = input(user, "Which song list?", "\The [src]") as null | anything in list("OTHERWORLDLY"=MUSIC_TAVCAT_OTHERWORLDLY, "GENERIC"=MUSIC_TAVCAT_GENERIC, "OLDSCHOOL"=MUSIC_TAVCAT_OLDSCHOOL, "ALIZERIA"=ALIZERIA)
		playsound(loc, pick('sound/misc/keyboard_select (1).ogg','sound/misc/keyboard_select (2).ogg','sound/misc/keyboard_select (3).ogg','sound/misc/keyboard_select (4).ogg'), 100, FALSE, -1)
		user.visible_message(span_info("[user] presses a button on \the [src]."),span_info("I press a button on \the [src]."))
		...
		if(songlists_selection=="ALIZERIA")
			chosen_songlists_selection = ALIZERIA
