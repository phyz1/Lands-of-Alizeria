...
/obj/structure/mirror/attack_hand(mob/user)
	...
	if(!HAS_TRAIT(H, TRAIT_MIRROR_MAGIC))
		to_chat(H, span_warning("Вы смотрите в зеркало, но видите только свое обычное отражение."))
		if(HAS_TRAIT(user, TRAIT_BEAUTIFUL))
			H.add_stress(/datum/stressevent/beautiful)
			to_chat(H, span_smallgreen("Я отлично выгляжу!"))
			...
			if(HAS_TRAIT(H, TRAIT_XYLIX) && !H.has_status_effect(/datum/status_effect/buff/xylix_joy))
				...
				to_chat(H, span_info("Моя красота вызывает улыбку на лице и удачу в моём пути.!"))
		if(HAS_TRAIT(H, TRAIT_UNSEEMLY))
			to_chat(H, span_warning("Я выгляжу ужасно..."))
