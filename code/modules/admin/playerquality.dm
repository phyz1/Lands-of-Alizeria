...
/proc/format_pq_text(the_pq)
	if(the_pq >= 100)
		return "<span style='color: #ff2400;'>БОЖЕСТВЕННОЕ!</span>"
	if(the_pq >= 70)
		return "<span style='color: #00ff00;'>Легендарное!</span>"
	if(the_pq >= 50)
		return "<span style='color: #00ff00;'>Исключительное!</span>"
	if(the_pq >= 30)
		return "<span style='color: #47b899;'>Отличное!</span>"
	if(the_pq >= 10)
		return "<span style='color: #69c975;'>Хорошее!</span>"
	if(the_pq >= 5)
		return "<span style='color: #58a762;'>Неплохое</span>"
	if(the_pq >= -4)
		return "Normal"
	if(the_pq >= -30)
		return "<span style='color: #be6941;'>Плохое</span>"
	if(the_pq >= -70)
		return "<span style='color: #cd4232;'>Дерьмо</span>"
	if(the_pq >= -99)
		return "<span style='color: #e2221d;'>Мусор</span>"
	if(the_pq <= -100)
		return "<span style='color: #ff00ff;'>Ничтожество</span>"
	return "Normal"
