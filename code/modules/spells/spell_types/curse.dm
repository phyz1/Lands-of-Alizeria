...
/proc/curse_of_madness(mob/user, message)
	if(user) //in this case either someone holding a spellbook or a badmin
		to_chat(user, span_warning("Я наслал проклятие безумия с посланием \"[message]\"!"))
		message_admins("[ADMIN_LOOKUPFLW(user)] наслал проклятие безумия с посланием \"[message]\"!")
		log_game("[key_name(user)] наслал проклятие безумия с посланием \"[message]\"!")

	GLOB.curse_of_madness_triggered = message // So latejoiners are also afflicted.

	deadchat_broadcast("Станцию поразило <span class='name'>Проклятие Безумия</span>, разбивая рассудки ужасной тайной: \"<span class='big hypnophrase'>[message]</span>\"")
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		...
		if(H.anti_magic_check(TRUE, FALSE, TRUE))
			to_chat(H, span_notice("На мгновение меня охватывает странное чувство, но затем оно проходит."))
			continue
...
/proc/give_madness(mob/living/carbon/human/H, message)
	...
	to_chat(H, span_warning("Мой разум раскалывается!"))
	switch(rand(1,10))
		if(1 to 3)
