/obj/effect/proc_holder/spell/invoked/mirror_transform
	name = "Зеркальное превращение"
	desc = "Временно даёт возможность использовать зеркала для изменения внешности."
	clothes_req = FALSE
...
/obj/effect/proc_holder/spell/invoked/mirror_transform/cast(list/targets, mob/user)
	...
	H.visible_message(
		span_notice("Отражение [H] ненадолго мерцает."),
		span_notice("Вы чувствуете, как формируется связь с зеркалами...")
	)
...
/obj/effect/proc_holder/spell/invoked/mirror_transform/proc/remove_mirror_magic(mob/living/carbon/human/H)
	if(!QDELETED(H))
		...
		to_chat(H, span_warning("Ваша связь с зеркалами угасает."))
...
/proc/perform_mirror_transform(mob/living/carbon/human/H)
	...
	var/list/choices = list(
		...
		"Цвет ушей 2",
		"Цвет глаз",
		"Стиль лицевой растительности",
		"Цвет лицевой растительности",
		"Деталь лица",
		"Причёска",
		"Основной цвет волос",
		"Вторичный градиент волос",
		"Вторичный натуральный цвет волос",
		"Третий градиент волос",
		"Третий цвет краски волос",
		"Рога",
		"Цвет рогов",
		"Пенис",
		"Размер пениса",
	...
	var/chosen = input(H, "Что изменить?", "Внешность") as null|anything in choices
	if(!chosen)
		return
	...
	switch(chosen)
		...
		if("Причёска")
			...
			var/new_style = input(H, "Выберите причёску", "Причёска") as null|anything in valid_hairstyles
			if(new_style)
		...
		if("Основной цвет волос")
			var/new_hair_color = color_pick_sanitized(H, "Выберите цвет волос", "Основной цвет волос", H.hair_color)
			if(new_hair_color)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
					var/datum/customizer_choice/bodypart_feature/hair/head/humanoid/hair_choice = CUSTOMIZER_CHOICE(/datum/customizer_choice/bodypart_feature/hair/head/humanoid)
		...
		if("Цвет лицевой растительности")
			var/new_facial_hair_color = color_pick_sanitized(H, "Выберите цвет лицевой растительности", "Цвет лицевой растительности", H.facial_hair_color)
		...
		if("Цвет глаз")
			var/new_eye_color = color_pick_sanitized(H, "Выберите цвет глаз", "Цвет глаз", H.eye_color)
			if(new_eye_color)
		...
		if("Вторичный градиент волос")
			var/datum/customizer_choice/bodypart_feature/hair/head/humanoid/hair_choice = CUSTOMIZER_CHOICE(/datum/customizer_choice/bodypart_feature/hair/head/humanoid)
			var/list/valid_gradients = list()
			for(var/gradient_type in GLOB.hair_gradients)
				valid_gradients[gradient_type] = gradient_type

			var/new_style = input(H, "Выберите натуральный градиент", "Вторичный натуральный градиент волос") as null|anything in valid_gradients
			if(new_style)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
					var/datum/bodypart_feature/hair/head/current_hair = null
		...
		if("Вторичный натуральный цвет волос")
			var/new_gradient_color = color_pick_sanitized(H, "Выберите цвет натурального градиента", "Цвет вторичного натурального градиента", H.hair_color)
		...
		if("Третий градиент волос")
			var/datum/customizer_choice/bodypart_feature/hair/head/humanoid/hair_choice = CUSTOMIZER_CHOICE(/datum/customizer_choice/bodypart_feature/hair/head/humanoid)
			var/list/valid_gradients = list()
			for(var/gradient_type in GLOB.hair_gradients)
				valid_gradients[gradient_type] = gradient_type

			var/new_style = input(H, "Выберите градиент краски", "Третий градиент краски") as null|anything in valid_gradients
			if(new_style)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
					var/datum/bodypart_feature/hair/head/current_hair = null
		...
		if("Третий цвет краски волос")
			var/new_gradient_color = color_pick_sanitized(H, "Выберите третий градиентный цвет волос", "Третий градиентный цвет", H.hair_color)
			if(new_gradient_color)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
					var/datum/customizer_choice/bodypart_feature/hair/head/humanoid/hair_choice = CUSTOMIZER_CHOICE(/datum/customizer_choice/bodypart_feature/hair/head/humanoid)
		...
		if("Стиль лицевой растительности")
			var/datum/customizer_choice/bodypart_feature/hair/facial/humanoid/facial_choice = CUSTOMIZER_CHOICE(/datum/customizer_choice/bodypart_feature/hair/facial/humanoid)
			...
			var/new_style = input(H, "Выберите стиль лицевой растительности", "Причёска") as null|anything in valid_facial_hairstyles
			if(new_style)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
					var/datum/bodypart_feature/hair/facial/current_facial = null
		...
		if("Аксессуар")
			...
			var/list/valid_accessories = list("нет")
			...
			var/new_style = input(H, "Выберите аксессуар", "Стиль аксессуара") as null|anything in valid_accessories
			if(new_style)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
		...
		if("Деталь лица")
			...
			var/list/valid_details = list("нет")
			...
			var/new_detail = input(H, "Выберите деталь лица", "Деталь лица") as null|anything in valid_details
			if(new_detail)
				var/obj/item/bodypart/head/head = H.get_bodypart(BODY_ZONE_HEAD)
				if(head && head.bodypart_features)
		...
		if("Пенис")
			var/list/valid_penis_types = list("нет")
			...
			var/new_style = input(H, "Выберите тип пениса", "Настройка пениса") as null|anything in valid_penis_types
			if(new_style)
				if(new_style == "нет")
		...
		if("Яички")
			var/list/valid_testicle_types = list("нет")
			...
			var/new_style = input(H, "Выберите тип яичек", "Настройка яичек") as null|anything in valid_testicle_types
			if(new_style)
				if(new_style == "нет")
		...
		if("Количество грудей")
			var/list/valid_breast_types = list("нет")
			...
			var/new_style = input(H, "Выберите тип груди", "Настройка груди") as null|anything in valid_breast_types
			if(new_style)
				if(new_style == "нет")
		...
		if("Вагина")
			var/list/valid_vagina_types = list("нет", "human", "hairy", "spade", "furred", "gaping", "cloaca")
			var/new_style = input(H, "Выберите тип вагины", "Настройка вагины") as null|anything in valid_vagina_types
			if(new_style)
				if(new_style == "нет")
					...
					var/new_color = color_pick_sanitized(H, "Выберите цвет вагины", "Цвет вагины", vagina.color || H.dna.features["mcolor"])
					if(new_color)
		...
		if("Размер груди")
			var/list/breast_sizes = list("Плоская", "Небольшая", "Маленькая", "Средняя", "Большая", "Объёмная", "Тяжёлая", "Массивная", "Огромная", "Непристойная")
			var/new_size = input(H, "Выберите размер груди", "Размер груди") as null|anything in breast_sizes
			if(new_size)
				var/obj/item/organ/breasts/breasts = H.getorganslot(ORGAN_SLOT_BREASTS)
				if(breasts)
					var/size_num
		...
		if("Размер пениса")
			var/list/penis_sizes = list("маленький", "средний", "большой")
			var/new_size = input(H, "Выберите размер пениса", "Размер пениса") as null|anything in penis_sizes
			if(new_size)
				var/obj/item/organ/penis/penis = H.getorganslot(ORGAN_SLOT_PENIS)
				if(penis)
					var/size_num
		...
		if("Размер яичек")
			var/list/testicle_sizes = list("маленькие", "средние", "большие")
			var/new_size = input(H, "Выберите размер яичек", "Размер яичек") as null|anything in testicle_sizes
			if(new_size)
				var/obj/item/organ/testicles/testicles = H.getorganslot(ORGAN_SLOT_TESTICLES)
				if(testicles)
					var/size_num
		...
		if("Хвост")
			...
			var/new_style = input(H, "Выберите хвост", "Настройка хвоста") as null|anything in valid_tails
			if(new_style)
				if(new_style == "нет")
		...
		if("Цвет хвоста 1")
			var/obj/item/organ/tail/tail = H.getorganslot(ORGAN_SLOT_TAIL)
			if(tail)
				var/new_color = color_pick_sanitized(H, "Выберите основной цвет хвоста", "Цвет хвоста 1", "#FFFFFF")
				if(new_color)
					tail.Remove(H)
				...
				to_chat(H, span_warning("У вас нет хвоста!"))

		if("Цвет хвоста 2")
			var/obj/item/organ/tail/tail = H.getorganslot(ORGAN_SLOT_TAIL)
			if(tail)
				var/new_color = color_pick_sanitized(H, "Выберите вторичный цвет хвоста", "Цвет хвоста 2", "#FFFFFF")
				if(new_color)
					tail.Remove(H)
					var/list/colors = list()
					if(tail.accessory_colors)
		...
		if("Уши")
			...
			var/new_style = input(H, "Выберите уши", "Настройка ушей") as null|anything in valid_ears
			if(new_style)
				if(new_style == "нет")
		...
		if("Цвет ушей 1")
			var/obj/item/organ/ears/ears = H.getorganslot(ORGAN_SLOT_EARS)
			if(ears)
				var/new_color = color_pick_sanitized(H, "Выберите основной цвет ушей", "Цвет ушей 1", "#FFFFFF")
				if(new_color)
					ears.Remove(H)
					var/list/colors = list()
				...
				to_chat(H, span_warning("У вас нет ушей!"))

		if("Цвет ушей 2")
			var/obj/item/organ/ears/ears = H.getorganslot(ORGAN_SLOT_EARS)
			if(ears)
				var/new_color = color_pick_sanitized(H, "Выберите вторичный цвет ушей", "Цвет ушей 2", "#FFFFFF")
				if(new_color)
					ears.Remove(H)
					var/list/colors = list()
					if(ears.accessory_colors)
		...
		if("Рога")
			...
			var/new_style = input(H, "Выберите рога", "Настройка рогов") as null|anything in valid_horns
			if(new_style)
				if(new_style == "нет")
		...
		if("Цвет рогов")
			var/obj/item/organ/horns/horns = H.getorganslot(ORGAN_SLOT_HORNS)
			if(horns)
				var/new_color = color_pick_sanitized(H, "Выберите основной цвет рогов", "Цвет рогов", "#FFFFFF")
				if(new_color)
					horns.Remove(H)
					var/list/colors = list()
				...
				to_chat(H, span_warning("У вас нет рогов!"))

		if("Крылья")
			var/list/valid_wings = list("нет")
			...
			var/new_style = input(H, "Выберите крылья", "Настройка крыльев") as null|anything in valid_wings
			if(new_style)
				if(new_style == "нет")
		...
		if("Цвет крыльев")
			var/obj/item/organ/wings/wings = H.getorganslot(ORGAN_SLOT_WINGS)
			if(wings)
				var/new_color = color_pick_sanitized(H, "Выберите основной цвет крыльев", "Цвет крыльев 1", "#FFFFFF")
				if(new_color)
					wings.Remove(H)
					var/list/colors = list()
				...
				to_chat(H, span_warning("У вас нет крыльев!"))
