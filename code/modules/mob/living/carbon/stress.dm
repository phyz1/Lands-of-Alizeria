...
/mob/living/carbon/update_stress()
	...
	if(new_stress != oldstress)
		...
		if(diff_abs > 1)
			if(ascending)
				to_chat(src, span_smallred("Мне стало хуже..."))
				if(diff_abs > 2)
					if(!rogue_sneaking || alpha >= 100)
						play_stress_indicator()
			else
				to_chat(src, span_smallgreen("Мне стало лучше."))
				if(diff_abs > 2)
					if(!rogue_sneaking || alpha >= 100)
	...
	if(old_threshold != new_threshold)
		...
		switch(new_threshold)
			if(STRESS_THRESHOLD_NICE)
				to_chat(src, span_green("Чувствую себя просто замечательно!"))
				apply_status_effect(/datum/status_effect/mood/vgood)
			if(STRESS_THRESHOLD_GOOD)
				if(ascending)
					to_chat(src, span_info("Мне стало хуже."))
				else
					to_chat(src, span_green("Чувствую себя отлично."))
				apply_status_effect(/datum/status_effect/mood/good)
			if(STRESS_THRESHOLD_NEUTRAL)
				if(ascending)
					to_chat(src, span_info("Мне стало хуже."))
				else
					to_chat(src, span_info("Мне стало лучше."))
			if(STRESS_THRESHOLD_STRESSED)
				if(ascending)
					to_chat(src, span_red("Мне стало хуже..."))
				else
					to_chat(src, span_red("Мне стало немного лучше."))
			...
			if(STRESS_THRESHOLD_STRESSED_BAD)
				if(!HAS_TRAIT(src, TRAIT_EORAN_CALM) && !HAS_TRAIT(src, TRAIT_EORAN_SERENE))
					if(ascending)
						to_chat(src, span_boldred("Мне становится слишком плохо..."))
					else
						to_chat(src, span_boldred("Похоже, всё не так плохо."))
					apply_status_effect(/datum/status_effect/mood/vbad)
			if(STRESS_THRESHOLD_FREAKING_OUT)
				to_chat(src, span_boldred("МНЕ УЖАСНО ПЛОХО!!!"))
...
/mob/living/carbon/proc/stress_freakout()
	to_chat(src, span_boldred("ПАНИКА!!!"))
	Stun(2 SECONDS)
	blur_eyes(2)
	freakout_hud_skew()
