...
/datum/species/demihuman
	...
	id = "demihuman"
	is_subrace = TRUE
	origin_default = /datum/virtue/origin/racial/aliz/jantar
	origin = "Жантар"
	base_name = "Человек"
	sub_name = "Зверолюд"
	desc = "<b>Зверолюд</b><br>\
	Порой, ярые последователи Дендора настолько отдаются своей 'дикой' страсти, что даже обезумевший бог подобное замечает. В результате на свет может появиться зверолюд - помесь человека и животного. \
	Среди лесных дикарей подобные особи являются диковинкой, а среди яростных дендоровцев - божьими детьми. В городах же к подобным относятся чуть лучше, \
	чем к полукровкам, т.к. их зачатие хотя бы является относительно богоугодным процессом, в отличие от зачатия полукровок.<br> \
